 ...
 * Changelog:
 * - making the layout to center the contained items
...
public class BubbleCloudView<T extends Adapter> extends AdapterView<T> {
    ...
    private void endTouch() {
        if (mAdapter.getCount() > 10) {
            if (scrollX > screenW || scrollX < -1 * screenW || scrollY > screenH || scrollY < -1 * screenH) {
                int veloX = deltaX;
                int veloY = deltaY;
    ...
    public ArrayList<Integer> correction(View view, int centerX, int centerY) {
        ArrayList<Integer> returnVal = new ArrayList<>();
        int mX = scrollX;
        int mY = scrollY;
        if (view.getX() > centerX) {
            mX -= (view.getX() - centerX);
            mX -= 30;
        } else if (view.getX() < centerX) {
            mX += (centerX - view.getX());
            mX -= 30;
        }
        if (view.getY() > centerY) {
            mY -= (view.getY() - centerY);
            if (view.getY() > centerY + 40) {
                mY += 30;
            }
        } else if (view.getY() < centerY) {
            mY += (centerY - view.getY());
            if (view.getY() < centerY - 40) {
                mY -= 40;
            }
        }
    ...
    public void iconMapRefresh(float sphereR, float hexR, float scrollX, float scrollY) {
        hexCubeOrtho.clear();
        hexCubePolar.clear();
        hexCubeSphere.clear();
...
//        Log.d("BubbleCloudView", "iconMapRefresh:resultMap1====" + hexCubeSphere);
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            BubbleCloudView a = (BubbleCloudView) findViewById(R.id.my_list);
            mScaleFactor = Math.max(1f, Math.min(mScaleFactor, 3.0f));
            a.setScaleX(mScaleFactor);
            a.setScaleY(mScaleFactor);
            return true;
        }
    }
